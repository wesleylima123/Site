...
(function () {
    ...
    function t2AvailableForT1(t1) {
        const selected = new Set(state.t1);
        return T2.filter(skill => prereqSatisfied(skill, selected) && !state.t2[t1.id]?.includes(skill.id));
    }

    function t3AvailableForBranch(t1) {
        const branchT2 = new Set(state.t2[t1.id] || []);
        const allSelectedT2 = new Set(Object.values(state.t2).flat());
        return T3.filter(skill => {
            if (skill.id === '3.20') return Object.keys(state.t3).length >= 3;
            return prereqSatisfied(skill, allSelectedT2) && prereqSatisfied(skill, branchT2);
        });
    }

    function selectedT1Skill(id) { return byId[id]; }
